...
<div class="question-content flex flex-col items-center gap-4 rounded-md">
	...
	<div id="question" class="question-text-container mt-20 relative w-full px-3 py-4 sm:p-4">
		...
		<div class="flex justify-end mb-4 sm:mb-6">
			<div class="relative group">
				<button
					class="w-12 h-8 rounded-full flex items-center justify-center transition-colors duration-300 focus:outline-none border-2 border-gray-600 hover:border-yellow-400 sm:w-10 sm:h-10"
					style="background-color: {$examStore.showSolution ? '#fbbf24' : '#374151'};"
					aria-label="Ver cómo resolverlo"
					on:click={toggleSolution}
				>
					<Eye class="w-4 h-4 sm:w-5 sm:h-5 text-gray-900" />
				</button>
				<!-- Tooltip creativo -->
				<div
					class="absolute right-0 top-0 mt-6 w-56 p-2 bg-gray-900 text-yellow-300 text-xs rounded-lg shadow-lg opacity-0 group-hover:opacity-100 group-hover:translate-y-0 -translate-y-2 pointer-events-none transition-all duration-300 z-10 select-none sm:mt-8 sm:w-64 sm:p-3"
				>
		...
		<div
			class="flex flex-wrap question-imgreference w-full justify-center items-center gap-2 sm:gap-4 min-h-[50px] mt-4"
		>

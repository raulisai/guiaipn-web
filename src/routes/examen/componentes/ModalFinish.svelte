<script>
	...
	onMount(async () => {
		if (browser) {
			...
			// Enviar examen a la API
			try {
				const {
					data: { session }
				} = await supabase.auth.getSession();
				if (session) {
					const answersList = Object.values($examStore.answersDetailed).map((a) => ({
						question_id: a.reactivoId, // Usamos reactivoId como question_id
						user_answer: a.userAnswer
					}));

					if (answersList.length > 0) {
						const result = await questionsAPI.submitExam(
...
>
	...
	>
		...
		<div class="text-center mb-8">
			<h2
				class="text-2xl md:text-3xl font-bold mb-2 bg-gradient-to-r from-cyan-400 to-blue-500 text-transparent bg-clip-text"
			>
				¡Examen Finalizado!
			</h2>
			...
			<div class="flex flex-col items-center">
				...
				<div class="text-center mt-4 space-y-2">
					<p class="text-lg text-gray-300">
						Respuestas correctas: <span class="text-cyan-400 font-bold">{correctAnswers}</span> de
